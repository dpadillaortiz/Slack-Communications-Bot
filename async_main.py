...
async def handle_some_action(ack, client, body, logger):
    await ack()
    logger.info(body)
...
async def handle_view_submission_events(ack, body, client, logger):
    await ack()
    logger.info(body)
...
async def handle_global_shortcut(ack, body, client, logger):
    await ack() 
    try:
        await client.views_open(
            trigger_id=body["trigger_id"],
...
async def send_windows_message(email: str, schedules: dict[str:str]):
    # TODO edit the get_block_message function to add the schedule values
    try:
